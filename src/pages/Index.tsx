...
import { playSpinSound, playWinSound, playButtonClick } from "@/utils/sound";
...
export default function Index() {
  ...
  const handleSpin = useCallback(() => {
    ...
    setTimeout(() => sound(playSpinSound), 100);
  }, [winnerCount, remainingNumbers, isMuted, sound]);
    ...
    <div className="min-h-screen" style={{ background: 'var(--gradient-warm)' }}>
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        {['🌿', '🍃', '🌸', '✨', '🍀', '🌿'].map((emoji, i) => (
          <span
            key={i}
            className="absolute text-2xl float opacity-30"
            style={{
              top: `${10 + i * 14}%`,
      ...
      >
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center justify-between mb-2">
            <button
              onClick={() => { sound(playButtonClick); setIsMuted(!isMuted); }}
              className="rounded-full p-2 transition-opacity hover:opacity-80"
              style={{ background: 'hsla(38, 55%, 95%, 0.15)', color: 'hsl(38, 55%, 95%)' }}
            ...
            <button
              onClick={() => { sound(playButtonClick); setShowHistory(!showHistory); }}
          ...
          <div className="mt-3">
            ...
            <div
              ...
              style={{ background: 'hsla(38, 55%, 95%, 0.2)' }}
            >
              <div
                ...
                style={{
                  ...
                  boxShadow: '0 0 8px hsl(43 96% 56% / 0.6)',
                }}
              />
            </div>
          </div>
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      disabled={isSpinning}
                      className="flex-1 rounded-xl px-4 py-3 text-center text-xl font-bold border-2 outline-none transition-all"
                      style={{
                    ...
                    {[1, 3, 5, 10].map((n) => (
                      <button
                        key={n}
                        onClick={() => { setWinnerCount(String(n)); setInputError(""); sound(playButtonClick); }}
                        disabled={isSpinning || n > remainingNumbers.length}
                        className="rounded-xl px-3 py-2 text-sm font-bold transition-all hover:scale-105 disabled:opacity-40"
                        style={{
                          background: winnerCount === String(n) ? 'hsl(var(--primary))' : 'hsl(var(--secondary))',
                          color: winnerCount === String(n) ? 'hsl(var(--primary-foreground))' : 'hsl(var(--foreground))',
                        }}
                      >
                        {n}
                      </button>
                  ...
                  {inputError && (
                    <p className="text-xs mt-1 font-semibold" style={{ color: 'hsl(var(--destructive))' }}>
                      ⚠️ {inputError}
                    </p>
                  )}
                </div>
                ...
                <button
                  onClick={handleReset}
                  disabled={isSpinning}
                  className="w-full rounded-full py-3 font-bold text-sm transition-all hover:opacity-80 flex items-center justify-center gap-2 disabled:opacity-40"
                  style={{
                    background: 'hsl(var(--secondary))',
              ...
              {/* Fun stats */}
              <div
                className="mt-4 rounded-2xl p-3 grid grid-cols-3 gap-3 text-center"
                style={{ background: 'hsl(var(--secondary))' }}
              >
                <div>
                  <div className="text-2xl">🏆</div>
                  <div className="text-lg font-display" style={{ color: 'hsl(var(--primary))' }}>{totalWon}</div>
                  <div className="text-xs" style={{ color: 'hsl(var(--muted-foreground))' }}>Đã trúng</div>
                </div>
                <div>
                  <div className="text-2xl">🎡</div>
                  <div className="text-lg font-display" style={{ color: 'hsl(var(--primary))' }}>{round}</div>
                  <div className="text-xs" style={{ color: 'hsl(var(--muted-foreground))' }}>Lượt quay</div>
                </div>
                <div>
                  <div className="text-2xl">🎯</div>
                  <div className="text-lg font-display" style={{ color: 'hsl(var(--primary))' }}>{remainingNumbers.length}</div>
                  <div className="text-xs" style={{ color: 'hsl(var(--muted-foreground))' }}>Còn lại</div>
                </div>
              </div>
            </div>
